...
public class SqlServerServerResource : ContainerResource, IResourceWithConnectionString
{
    ...
    private ReferenceExpression BuildConnectionString()
    {
        ...
        builder.Append($"Server={PrimaryEndpoint.Property(EndpointProperty.IPV4Host)},{PrimaryEndpoint.Property(EndpointProperty.Port)};User ID={DefaultUserName};Password={PasswordParameter}");
    ...
    /// </summary>
    /// <remarks>
    /// <para>Format: <c>jdbc:sqlserver://{Host}:{Port}[;databaseName={Database}];encrypt=true</c> when TLS is enabled,
    /// or <c>jdbc:sqlserver://{Host}:{Port}[;databaseName={Database}];trustServerCertificate=true</c> otherwise.</para>
